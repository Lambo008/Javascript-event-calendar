...
(function (factory) {
  ...
  var clndrTemplate =
    "<div class='clndr-controls'>" +
        ...
        '</div>' +
        "<div class='month display-4 mb-4 text-center'><%= month %> <%= year %></div>" +
    ...
    "<table class='clndr-table' border='0' cellspacing='0' cellpadding='0'>" +
        '<thead>' +
            "<tr class='header-days bg-dark text-white'>" +
        ...
        '</tbody>' +
    '</table>'
    // "<div id='clndr-event-listing' class='clndr-event-list'>" +
    //     "<h2 class='upcoming-events'>Upcoming Events</h2>" +
    //     "<button type='button' id='multi-event-button' class='btn btn-primary multi-event' data-toggle='modal' data-target='#Multi-days-event-modal'>Add Multi-days Event</button>"+
    // '</div>'
    ;
  ...
  Clndr.prototype.createDayObject = function (day, monthEvents) {
    ...
    if (eventsToday.length) {
      eventsToday.forEach(element => {
        if(element.endDate){
          extraClasses += (' ' + this.options.classes.multi_event);
        }else{
